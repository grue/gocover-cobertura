...
func (cov *Coverage) parseProfile(profile *Profile, pkgPkg *packages.Package, ignore *Ignore) error {
	...
	// TODO(grue): package paths are not file paths, there is a consistent separator
...
func (v *fileVisitor) class(n *ast.FuncDecl) *Class {
	...
	if byFiles {
		...
		//
		// NOTE(grue): ReportGenerator creates links that collide if names are not distinct.
